...
{
	m_cc_bmp->SetBitmap(HR_ico(1));
	ok_btn->SetLabel(local->get("auth_f", "ok_btn", "Ok"));
	cancel_btn->SetLabel(local->get("auth_f", "cancel_btn", "Cancel"));
	message_lb->SetLabel(cc_main_caption_);
	show_kbd_btn->SetLabel(local->get("auth_f", "show_kbd_btn1", "Show keyboard"));
	login_lb->SetLabel(local->get("auth_f", "login_lb", "login"));
	password_lb->SetLabel(local->get("auth_f", "password_lb", "password"));
	host_lb->SetLabel(local->get("auth_f", "host_lb", "host"));
...
void auth_f::onChar( wxKeyEvent& event ) {
	...
	if (event.GetKeyCode() == WXK_TAB) {
		...
		if (half_show_ && catch_return_) {
			catch_return_ = false;
			if (!req_version_) 
				wxMessageBox(_("This magnetic card has obsolete data format, and it is no longer supported.  Please renew card using HR Manager v1.0.1.15 or newer. Or use virtual keyboard to type login and password."));
...
void auth_f::show_kbd_btnClick( wxCommandEvent& event )
{
	...
	if (!kbd_showed_) {
		...
		show_kbd_btn->SetLabel(local->get("auth_f", "show_kbd_btn2", "Hide keyboard"));
...
void auth_f::full_show() {
	advanced_btn->SetLabel(local->get("auth_f", "advanced_btn2", "Advanced <<"));
...
void auth_f::half_show() {
	advanced_btn->SetLabel(local->get("auth_f", "advanced_btn1", "Advanced >>"));
	//advanced_btn->SetLabel(_("Advanced >>"));
	ok_btn->Show(false);
...
bool auth_f::check() {
	if (login_ed->GetValue() == _("")) {
		wxMessageBox(local->get("auth_f", "empty_login_msg", "Empty login is not allowed"));
	}
	else if (password_ed->GetValue() == _("")) {
		wxMessageBox(local->get("auth_f", "empty_password_msg", "Empty password is not allowed"));
	}
	else if (host_ed->GetValue() == _("")) {
		wxMessageBox(local->get("auth_f", "empty_host_msg", "Empty host is not allowed"));
	}
	else {
		return true;
	}
