...
kitchen_t::kitchen_t(int kitchen_id_, const std::string& name_, bool enable_)
: kitchen_id_(kitchen_id_), name_(name_), enable_(enable_)
{
...
//---------------------------------------------------------------------------
container_kitchen_t::container_kitchen_t(const std::string& str)
{
  std::string par = str;
  std::stringstream tmp(full_trim(par));
  ...
  {
    tmp >> id_val;
    kitchen_id_vec.push_back(id_val);
  }
  ...
  while (!query->next())
  {
    ...
    kitchen_vec_.push_back(kitchen_t(kitchen_id, name, enabled));
  }
...
void order_dish_modifier_t::mod_register(int reg_count, const std::string& ver, int kitchen_id, bool auto_ready) const
{
  ...
  ksi_cafe::logical_transaction_t(ksi_cafe::session.get(), 104);
...
bool try_order_register(const order_t& order, const container_kitchen_t& container, const std::string& ver, bool auto_ready)
{
  ...
  for (int index = 0; index < container.count(); ++index)
  {
    ...
    {
			if (!order.dish(i).registered() &&  container.kitchen(index).kitchen_execute(order.table().table_group_id(), order.dish(i).kitchen_type_id()))
      {
...
//---------------------------------------------------------------------------
order_t::order_t(int order_id): order_id_(order_id)
{
  std::auto_ptr<ksi_client::oracle_query_i> query = ksi_cafe::session->create_query();
  ...
  std::auto_ptr<ksi_client::oracle_query_i> mquery = ksi_cafe::session->create_query();
  ...
  while (!query->next())
  {
    ...
    while (!mquery->next())
    {
      ...
      mod_vec.push_back(order_dish_modifier_t(order_dish_id, modifier_name, modifier_id, modifier_dish_id, kitchen_type_id, count, count_wasted, count_registered));
    }
    order_dish_vec_.push_back(order_dish_t(order_dish_id, dish_id, dish_name, kitchen_type_id, count, count_wasted, count_registered, description, time, mod_vec));
