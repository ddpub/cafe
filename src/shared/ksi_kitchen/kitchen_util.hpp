...
{
public:
  kitchen_t(int kitchen_id_, const std::string& name_, bool enable_);
public:
...
{
public:
  explicit container_kitchen_t(const std::string& str);
...
{
public:
  order_dish_modifier_t(int order_dish_id_, const std::string name_, int modifier_id_, int mod_dish_id_
                      , int kitchen_type_id_, int count_, int count_wasted_, int count_registered_)
  : order_dish_id_(order_dish_id_), name_(name_), modifier_id_(modifier_id_), mod_dish_id_(mod_dish_id_)
...
{
public:
  order_dish_t(int order_dish_id_, int dish_id_, const std::string& name_, int kitchen_type_id_, int count_, int count_wasted_
             , int count_registered_, const std::string& description_, boost::posix_time::ptime time_, const std::vector<order_dish_modifier_t>& mods_)
  : order_dish_id_(order_dish_id_), dish_id_(dish_id_), name_(name_), kitchen_type_id_(kitchen_type_id_), count_(count_)
  , count_wasted_(count_wasted_), count_registered_(count_registered_), description_(description_), time_(time_), mods_(mods_) {}
public:
  ...
  bool modifier_registered() const
  {
    for (int i=0; i<mod_count(); i++)
      if (!modifier(i).registered(total_count()))
        return false;
    return true;
  }
public:
