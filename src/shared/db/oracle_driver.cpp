...
void database::oracle::server::check_for_error(int retval, bool ignore_no_data)
{
  if (OCI_SUCCESS == retval)
    return;
  if (OCI_SUCCESS_WITH_INFO == retval)
    return;
  if (OCI_NO_DATA == retval && ignore_no_data)
    return;
  if (retval >= 12150 && retval <= 12236) // TNS ERROR
	  throw TNS_error(retval, __err.hndl);
...
session& database::oracle::session::duplicate()
{
	boost::shared_ptr<session> sess_ptr (new session(*this));
	__duplicates.push_back(sess_ptr);
	return *(__duplicates.back());
}
...
DEFINE_STORAGE_TYPE(datatypes::varraw, SQLT_VBI, vector<unsigned char>)
typedef __define_storage_info<datatypes::varraw>::storage_type varraw_storage;
...
template<>
void* ptr<varraw_storage>(varraw_storage& data)
{
  return &(data[0]);
}
template<>
...
template<>
size_t size<varraw_storage>(varraw_storage& data)
{
  return data.size();
}
template<>
...
template<>
inline varraw_storage* allocate<varraw_storage>(size_t size, database::oracle::session* sess)
{
  return new varraw_storage(size + 2);
}
template<>
                                           ...
                                           , unsigned int pos)
{
  ...
  case OCI_TYPECODE_VARCHAR2:
    define_storage.reset(new __define_datatype<datatypes::varchar>(length+1, &__session));
    break;
...
void database::oracle::query::force_varraw(unsigned index)
{
    size_t size = __define_container[index]->get_data_size();
    __define_container[index].reset(new __define_datatype<datatypes::varraw>(size, &__session));
    __define_container_t::value_type& define_storage = __define_container[index];
    OCIDefine* defhp;
    check_for_error(OCIDefineByPos(__stmt.hndl, &defhp, __session.errhp()
                                   , index + 1
                                   , define_storage->get_ptr_for_define()
...
#ifdef _SQL_DEBUG_
	sql_ = statement;
	origin_sql_ = statement;
#endif
	__has_out_to_fetch = false;
  __stmt.reset();
  __clear_containers();
...
extract_dynamic(query::__define_container_t& data, unsigned int index, size_t row)
{
  ...
  if (base.get_datatype() != Data_Type)
  {
    stringstream err_ss;
    err_ss << "Incorrect datatype. Requested type: " << Data_Type
           << ". Column type is: " << base.get_datatype();
    throw oracle::exception(err_ss.str());
  }
...
inline short cast_oci_number(OCINumber number, T& target
                             , bool unsigned_flag, bool float_flag, bool to_str_flag
                             , OCIError* errhp)
{
  return float_flag
...
template <typename T>
inline T query::__get_numeric(unsigned int index
                              ...
                              , bool to_str_flag)
{
  ...
  if (__define_container[index]->is_null(__current_position))
    return 0;
  if (column_datatype(index) == datatypes::varchar)
  {
    __define_datatype<datatypes::varchar>::container_type& data =
      extract<datatypes::varchar>(__define_container, index, __current_position);
    string target_str = &data[0];
    return boost::lexical_cast<T>(target_str);
  }
  check_for_error(cast_oci_number<T>(
                    extract<datatypes::numeric>(__define_container, index, __current_position)
                    , target
                    , unsigned_flag, float_flag, to_str_flag
                    , __session.errhp()));
  return target;
}
...
std::string database::oracle::query::get_string(unsigned int column_index)
{
  datatypes::datatype type = __define_container[column_index]->get_datatype();
  if (datatypes::varchar == type)
  {
  ...
  if (datatypes::clob == type)
  {
    __define_datatype<datatypes::clob>::container_type& data =
      extract<datatypes::clob>(__define_container, column_index, __current_position);
    return string(&data.fetch()[0]);
  }
  if (datatypes::numeric == type)
  {
    long double value = 0;
    value = __get_numeric<long double>(column_index, false, true, true);
    return boost::lexical_cast<string>(value);
  }
  throw;
}

std::pair<size_t, unsigned char*> database::oracle::query::get_varraw(unsigned int column_index)
{
    if (datatypes::varraw != __define_container[column_index]->get_datatype())
        throw "get_varraw apllyable only for force_varraw'ed fields";
    __define_datatype<datatypes::varraw>::container_type& data =
        extract<datatypes::varraw>(__define_container, column_index, __current_position);
    unsigned short size = (data[1] << 8) | data[0];
    return std::pair<size_t, unsigned char*>(size, &(data[2]));
}
...
#ifdef _SQL_DEBUG_
	void bind_str(std::string& sql_, const std::string& fieldName, const std::string& value) {
		size_t pos = sql_.find(":" + fieldName);
		if (pos == std::string::npos) {}//throw std::runtime_error("Cannot find Field: \"" + fieldName + "\", in statement \n" + sql_);
		else sql_.replace(pos, fieldName.length() + 1, "'" + value + "'");
	}
	template <typename T> void bind_val(std::string& sql_, const std::string& fieldName, T value) {
		size_t pos = sql_.find(":" + fieldName);
		if (pos == std::string::npos) {}//throw std::runtime_error("Cannot find Field: \"" + fieldName + "\", in statement \n" + sql_);
		else {
      std::string value_str = boost::lexical_cast<std::string>(value);
      sql_.replace(pos, fieldName.length() + 1, value_str);
    }
	}
#endif
...
void database::oracle::query::set(const std::string& param_position, long unsigned int data)
{
  __set_int(param_position, data);
}
...
{
#ifdef _SQL_DEBUG_
	bind_val(sql_, param_position, data);
#endif
...
{
#ifdef _SQL_DEBUG_
	bind_str(sql_, param_position, data);
#endif
...
void database::oracle::query::set(const std::string& param_position, const char* data)
{
	set(param_position, std::string(data));
}
...
{
	size_t type = data.second.which();
  switch (type)
  {
  case 0:
    cast_oci_number(*data.first, *boost::get<char*>(data.second), false, false, false, __session.errhp());
		break;
  case 1:
    cast_oci_number(*data.first, *boost::get<short*>(data.second), false, false, false, __session.errhp());
		break;
  case 2:
    cast_oci_number(*data.first, *boost::get<int*>(data.second), false, false, false, __session.errhp());
		break;
  case 3:
    cast_oci_number(*data.first, *boost::get<long long*>(data.second), false, false, false, __session.errhp());
		break;
  case 4:
    cast_oci_number(*data.first, *boost::get<unsigned char*>(data.second), true, false, false, __session.errhp());
		break;
  case 5:
    cast_oci_number(*data.first, *boost::get<unsigned short*>(data.second), true, false, false, __session.errhp());
		break;
  case 6:
    cast_oci_number(*data.first, *boost::get<unsigned int*>(data.second), true, false, false, __session.errhp());
		break;
  case 7:
    cast_oci_number(*data.first, *boost::get<unsigned long long*>(data.second), false, false, false, __session.errhp());
		break;
  case 8:
    cast_oci_number(*data.first, *boost::get<long unsigned int*>(data.second), false, false, false, __session.errhp());
		break;
  case 9:
    cast_oci_number(*data.first, *boost::get<float*>(data.second), false, true, false, __session.errhp());
		break;
  case 10:
    cast_oci_number(*data.first, *boost::get<double*>(data.second), false, true, false, __session.errhp());
		break;
  case 11:
    cast_oci_number(*data.first, *boost::get<long double*>(data.second), true, false, false, __session.errhp());
		break;
  }
}
