...
#define MAX_FETCH_BUFFER_SIZE 10000
#endif
...
#define EXECUTE_AND_CATCH(QUERY, log) 			\
try { 										\
	QUERY.execute(); 						\
} 											\
catch (database::oracle::exception&) { 		\
	log << QUERY.sql() << std::endl; 	\
	throw; \
}
...
#define ORACLE_DRIVER_HANDLE(TYPE, VALUE)                                     \
    template <>                                                 \
    inline unsigned short get_htype<TYPE>() { return VALUE; }; 

    ORACLE_DRIVER_HANDLE(OCIServer, OCI_HTYPE_SERVER)
    ORACLE_DRIVER_HANDLE(OCIError, OCI_HTYPE_ERROR)
    ORACLE_DRIVER_HANDLE(OCIStmt, OCI_HTYPE_STMT)
    ORACLE_DRIVER_HANDLE(OCISvcCtx, OCI_HTYPE_SVCCTX)
    ORACLE_DRIVER_HANDLE(OCISession, OCI_HTYPE_SESSION)
    ORACLE_DRIVER_HANDLE(OCIDateTime, OCI_DTYPE_TIMESTAMP)
    ...
    struct __handle_allocation_manager<OCIEnv>
    {
      static void alloc(OCIEnv** hndl, OCIEnv** env)
        {
          OCIEnvInit(env, OCI_THREADED, 0, NULL);
          *hndl = *env;
        }
    ...
    public:
      ...
      exception(const exception& src)
                : std::exception(src)
                , __statement(src.__statement)
                , __code(src.__code)
                , __message(src.__message)
                , __complete_message(src.__complete_message)
                {
                }
	...
	{
	public:
	  connection_error(int oci_return_value, OCIError* errhp)
		  		: exception(oci_return_value,errhp)
	  {
	  }
	};

	class TNS_error : public connection_error
	{
	public:
	  TNS_error(int oci_return_value, OCIError* errhp)
		  		: connection_error(oci_return_value,errhp)
	  {
	  }
	};
    ...
    public:
      ...
      void detach();
      void check_for_error(int retval, bool ignore_no_data = false);
      virtual ~server() {
      }
    };
    ...
    class session
    {
      ...
      __handle<OCISvcCtx> __ctx;
			std::list<boost::shared_ptr<session> > __duplicates;
			std::string __user;
			std::string __pass;
    public:
      explicit session(server& srv)
        : __server(srv)
        ...
        , __ctx(__server.envhp())
        {
			...
			virtual ~session()
				{
					OCITransRollback(__ctx.hndl, __server.errhp(), 0);
				}
    public:
    ...
    public:
      ...
      OCISvcCtx* ctxhp()
		...
		private:
			session(const session& target)
				: __server(target.__server)
				, __sess(__server.envhp())
				, __ctx(__server.envhp())
				{
					if (target.__user != "" && target.__pass != "")
						login(target.__user, target.__pass);
				}
    };
    ...
    public:
      ...
      typedef boost::variant<char*, short*, int*, long long*
                             , unsigned char*, unsigned short*, unsigned int*, unsigned long long*, long unsigned int*
    ...
    private:
      ...
      template<typename T>
      inline T __get_numeric(unsigned int column_index
                             , bool unsigned_flag, bool float_flag, bool to_str_flag = false);
    ...
    bool prepared() const { return __prepared; }
...
#endif
      void force_varraw(unsigned index);
      ...
      void set(const std::string& param_position, unsigned char data);
			void set(const std::string& param_position, long unsigned int data);
	  ...
	  void set(const std::string& param_position, const char* data);
      void set(const std::string& param_position, unsigned long long* data);
      void set(const std::string& param_position, unsigned int* data);
      void set(const std::string& param_position, unsigned short* data);
      void set(const std::string& param_position, unsigned char* data);
      ...
      std::pair<size_t, unsigned char*> get_varraw(unsigned int column_index);
      std::string get_string(unsigned int column_index);
