...
// Maintainer:  $Author: dda $
// Created:     01/02/97
// RCS-ID:      $Id: treelistctrl.cpp,v 1.5 2009/12/11 13:59:37 dda Exp $
...
void wxTreeListMainWindow::PaintItem (wxTreeListItem *item, wxDC& dc) {
	...
	wxString wxEmptyString = _("");
	dc.GetTextExtent( item->GetText(GetMainColumn()).size() > 0
            ? item->GetText(GetMainColumn())
            : wxEmptyString,  // dummy text to avoid zero height and no highlight width
        &text_w, &text_h );
...
void wxTreeListMainWindow::CalculateSize (wxTreeListItem *item, wxDC &dc) {
    ...
    dc.GetTextExtent (item->GetText(m_main_column).size() > 0
            ...
            : wxEmptyString,  // blank to avoid zero height and no highlight width
        &text_w, &text_h);
    // restore normal font
    dc.SetFont (m_normalFont);
