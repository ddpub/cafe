...
$pageTitle = $id ? getLM("Tare group details") : "Adding a new tare group";

$basePage = "index.php";

$table = "ksi.tare_group";
$idField = "group_id";
$idNewVal = "ksi.tare_group_seq.nextval";
...
if ($_POST["submit"] == getLM("save")) {
	...
	if ($id) {
		updateItem($table, $arrFV, $idField, $id, 173);
	}
	else {
		$arrFV[] = array($idField, $idNewVal);
		/*$id = */insertItem($table, $arrFV, 172);
	}
...
function writeMainContent() {
	...
	$str = "";
	$tree = new rTree2("SELECT group_id, parent_id, title AS name FROM ksi.tare_group WHERE group_id <> " . (int)$GLOBALS["id"] . " ORDER BY NAME", $GLOBALS["objConn"]);
