...
$table = "ksi.ingredient_product";
$idField = "ingredient_product_id";
$idNewVal = "ksi.ingredient_product_seq.nextval";
...
/*
		...
		$strSQL = "UPDATE " . $table . " 
			...
			WHERE ingredient_id = " . (int)$_GET["i"] . " AND product_id = " . (int)$_GET["it"];
		...
		logTranEnd();
*/
		updateItem($table, $arrFV, $idField, $id, 14);
	}
	else {
		$arrFV[] = array($idField, $idNewVal);
		$arrFV[] = array("ingredient_id", (int)$_GET["i"]);
		$arrFV[] = array("product_id", (int)$_POST["product_id"]);
		insertItem($table, $arrFV, 13);
	}
...
if ($id) {
	$strSQL = "SELECT ksi.ingredient.name AS ingredient_name
			...
			, ksi.ingredient_product.*
		FROM ksi.ingredient 
			LEFT JOIN ksi.ingredient_product ON ksi.ingredient.ingredient_id = ksi.ingredient_product.ingredient_id
			LEFT JOIN ksi.product ON ksi.product.product_id = ksi.ingredient_product.product_id
		WHERE ksi.ingredient_product.ingredient_product_id = " . $id;
...
$pageTitle = $id ? "Details of " . $result["PRODUCT_NAME"] . " item, included in the " . $result["INGREDIENT_NAME"] . " ingredient" : "Adding an item to the " . $result["INGREDIENT_NAME"] . " ingredient";
...
function writeMainContent() {
	...
	writeValRow(getLM("Ingredient"), $GLOBALS["result"]["INGREDIENT_NAME"]);
