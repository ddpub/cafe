...
$table = "ksi.ingredients";
$idField = "ingredients_id";
$idNewVal = "ksi.ingredients_seq.nextval";
...
/*
		...
		$strSQL = "UPDATE " . $table . " 
			...
			WHERE base_ingredient_id = " . (int)$_GET["i"] . " AND ingredient_id = " . (int)$_GET["i2"];
		...
		logTranEnd();
*/
		updateItem($table, $arrFV, $idField, $id, 17);
	}
	else {
		$arrFV[] = array($idField, $idNewVal);
		$arrFV[] = array("base_ingredient_id", (int)$_GET["i"]);
		$arrFV[] = array("ingredient_id", (int)$_POST["ingredient_id"]);

		insertItem($table, $arrFV, 16);
	}
...
if ($id) {
	$strSQL = "SELECT ksi.ingredient.name AS ingredient_name
			,  i2.name AS ingredient2_name
			, ksi.ingredients.*
		FROM ksi.ingredient 
			LEFT JOIN ksi.ingredients ON ksi.ingredients.base_ingredient_id = ksi.ingredient.ingredient_id
			LEFT JOIN ksi.ingredient i2 ON i2.ingredient_id = ksi.ingredients.ingredient_id
		WHERE ksi.ingredients.ingredients_id = " . $id;
	...
	$strSQL = "SELECT ksi.ingredient.name AS ingredient_name
		FROM ksi.ingredient
		WHERE ksi.ingredient.ingredient_id = " . (int)$_GET["i"];
...
$pageTitle = $id ? "Details of " . $result["INGREDIENT2_NAME"] . " ingredient, included in the " . $result["INGREDIENT_NAME"] . " ingredient" : "Adding an ingredient to the " . $result["INGREDIENT_NAME"] . " ingredient";
...
function writeMainContent() {
	...
	writeValRow("Ingredient", $GLOBALS["result"]["INGREDIENT_NAME"]);
...
/*		if ($curGroup != $row["GROUP_ID"]) {
	...
	writeValRow(getLM("Item group"), "<select name=\"ingredient_id_g\"  onchange=\"fillItemC()\">" . $str . "</select>");
