...
$pageTitle = $id ? getLM("Item group details") : "Adding a new item group";

$basePage = "index.php";

$table = "ksi.product_group";
$idField = "group_id";
$idNewVal = "ksi.product_group_seq.nextval";
...
if ($_POST["submit"] == getLM("save")) {
	...
	if ($id) {
		updateItem($table, $arrFV, $idField, $id, 2);
	}
	else {
		$arrFV[] = array($idField, $idNewVal);
		/*$id = */insertItem($table, $arrFV, 1);
	}
...
function writeMainContent() {
	...
	$str = "";
	$tree = new rTree2("SELECT group_id, parent_id, name FROM ksi.product_group WHERE group_id <> " . (int)$GLOBALS["id"] . " ORDER BY pos, name", $GLOBALS["objConn"]);
